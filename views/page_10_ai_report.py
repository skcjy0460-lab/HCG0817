...
from lib.calculations import financial_diagnosis, checklist_progress, overall_risk_score
...
for key, cl in CHECKLISTS.items():
    ...
    progress[labels[key]] = checklist_progress(checked, cl)
...
risk = overall_risk_score(progress, fin_result["risk_level"], forbidden_flags)
...
st.download_button(
    ...
    file_name=f"{(st.session_state.get('org_name') or '병원')}_폐업해산파산_종합진단리포트.html",
    mime="text/html",
